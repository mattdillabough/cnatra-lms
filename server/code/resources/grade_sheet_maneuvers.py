...
class GradeSheetManeuvers(Resource):

    def get(self, grade_sheet_id):

        # get grade_sheet grade_sheet_maneuvers
        grade_sheet_maneuvers = [grade_sheet_maneuver.as_dict() for grade_sheet_maneuver in GradeSheetManeuverModel.objects(grade_sheet=grade_sheet_id)]

        # return grade_sheet_maneuvers or 404 (not found)
        if grade_sheet_maneuvers:
            return {'grade_sheet_maneuvers': grade_sheet_maneuvers}
        return {'message': 'Grade sheet maneuvers not found'}, 404
